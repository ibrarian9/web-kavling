...
<div class="space-y-4">
    ...
    <!-- Unified Table of Employee Salaries with CSS Table-to-Card Transformation -->
    <x-table :headers="['Nama Karyawan / Pekerja', 'Jabatan / Posisi', ['label' => 'Gaji Pokok', 'class' => 'p-3.5 text-right'], ['label' => 'Tunjangan', 'class' => 'p-3.5 text-right'], ['label' => 'Potongan', 'class' => 'p-3.5 text-right'], ['label' => 'Gaji Bersih (Net)', 'class' => 'p-3.5 text-right'], 'Rekening Bank', ['label' => 'Aksi Founder', 'class' => 'p-3.5 text-center']]" loadingTarget="search, gotoPage, nextPage, previousPage">
        @forelse($salaries as $sal)
            <tr class="hover:bg-slate-50 transition">
                ...
                <td data-label="Jabatan / Posisi" class="p-3.5 font-bold text-slate-800 text-xs">{{ $sal->position ?? '-' }}</td>
                <td data-label="Gaji Pokok" class="p-3.5 text-right font-mono font-bold text-xs">Rp {{ number_format($sal->basic_salary, 0, ',', '.') }}</td>
                <td data-label="Tunjangan" class="p-3.5 text-right font-mono text-emerald-700 font-bold text-xs">+Rp {{ number_format($sal->allowance, 0, ',', '.') }}</td>
                <td data-label="Potongan" class="p-3.5 text-right font-mono text-rose-600 font-bold text-xs">-Rp {{ number_format($sal->deductions, 0, ',', '.') }}</td>
                <td data-label="Gaji Bersih (Net)" class="p-3.5 text-right font-mono text-emerald-800 font-black text-sm">
                    Rp {{ number_format($sal->net_salary, 0, ',', '.') }}
                </td>
                <td data-label="Rekening Bank" class="p-3.5">
        ...
        @empty
            <x-table-empty colspan="8" title="Belum Ada Standar Gaji Karyawan Yang Ditetapkan" message="Klik tombol 'Tetapkan Gaji Karyawan' di atas untuk mendaftarkan komponen gaji staf atau pekerja." />
        @endforelse
    </x-table>
</div>
