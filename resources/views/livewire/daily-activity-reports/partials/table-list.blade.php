                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-4 py-3 text-center whitespace-nowrap">
                            <div class="flex items-center justify-center gap-1">
                                <button wire:click="showReportDetail({{ $rep->id }})" class="px-2 py-1 bg-slate-100 hover:bg-teal-50 hover:text-teal-700 font-bold rounded-lg text-[10px] text-slate-700 transition flex items-center gap-1" title="Lihat Rincian Laporan">
                                    <svg class="w-3.5 h-3.5 text-slate-500 hover:text-teal-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    <span>Detail</span>
                                </button>

                                @if(auth()->user()->isFounder() || auth()->user()->isSupervisor() || $rep->user_id === auth()->id())
                                    <button wire:click="editReport({{ $rep->id }})" class="p-1 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-lg transition" title="Edit Laporan">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </button>
                                    <button wire:click="deleteReport({{ $rep->id }})" wire:confirm="Yakin ingin menghapus laporan aktivitas prospek {{ $rep->client_name }}?" class="p-1 text-rose-600 hover:text-rose-800 hover:bg-rose-50 rounded-lg transition" title="Hapus Laporan">
