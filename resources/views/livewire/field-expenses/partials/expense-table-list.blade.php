...
<div class="card-clean p-4 border border-slate-200/80 rounded-3xl space-y-3 shadow-2xs">
    ...
    <!-- Unified Table of Field Expenses with CSS Table-to-Card Transformation -->
    <x-table :headers="['Tgl Transaksi', 'Kategori / Tipe', 'Proyek & Unit', 'Nama Rincian Barang / Worker', 'Jumlah Qty & Harga Unit', ['label' => 'Total Biaya', 'class' => 'p-3.5 text-right'], ['label' => 'Bukti Resi', 'class' => 'p-3.5 text-center'], ['label' => 'Aksi', 'class' => 'p-3.5 text-right']]" loadingTarget="project_id, unit_id, category_filter, search, datePeriod, startDate, endDate">
        @forelse ($expenses as $item)
            <tr class="hover:bg-slate-50/60 transition-colors">
                <td data-label="Tgl Transaksi" class="p-3.5 font-mono font-bold text-slate-700 text-xs whitespace-nowrap">
                    {{ format_id_date($item['date']) }}
                </td>
                <td data-label="Kategori / Tipe" class="p-3.5 whitespace-nowrap">
                    @if ($item['type'] === 'salary')
                ...
                <td data-label="Rincian" class="p-3.5 font-bold text-slate-900 text-xs">
                    {{ $item['title'] }}
                </td>
                <td data-label="Jumlah Qty & Harga" class="p-3.5 font-mono text-slate-700 text-xs">
                    <span class="bg-slate-100 px-2 py-0.5 rounded font-semibold text-slate-800">{{ $item['quantity_label'] }}</span>
                </td>
                <td data-label="Total Biaya" class="p-3.5 text-right font-mono font-extrabold text-slate-900 text-xs whitespace-nowrap">
                    Rp {{ number_format($item['total_price'], 0, ',', '.') }}
                </td>
                <td data-label="Bukti Resi" class="p-3.5 text-center whitespace-nowrap">
        ...
        @empty
            <x-table-empty colspan="8" title="Belum Ada Catatan Pengeluaran Lapangan" message="Data upah tukang & belanja material proyek akan dicantumkan di sini." />
        @endforelse
    </x-table>
