            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:flex lg:items-center gap-2.5 w-full lg:w-auto">
                ...
                @if(auth()->user()->isFounder() || auth()->user()->isSupervisor())
                    <button wire:click="openModal" 
                            class="w-full sm:col-span-2 lg:w-auto h-10 inline-flex items-center justify-center gap-2 px-4 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white text-xs font-semibold rounded-xl shadow-sm hover:shadow transition-all whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-emerald-500/30">
            ...
            <div class="card-clean p-5 space-y-4 flex flex-col justify-between hover:shadow-md transition">
                <div>
                    ...
                    @php
                        ...
                        if ($activeWorkers->isEmpty() && $unit->project) {
                            $activeWorkers = $unit->project->assignments->where('status', 'active');
                        }
