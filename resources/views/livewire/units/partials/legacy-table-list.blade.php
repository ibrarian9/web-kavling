...
<div class="card-clean overflow-hidden">
    ...
    <div class="p-4 border-b border-slate-100 space-y-3">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-1 text-xs">
            <!-- Search Input -->
            <div class="relative">
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari Kode Unit / Nama Pembeli..." class="input-clean w-full pl-9 pr-8 text-xs font-medium">
                <div class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                </div>
                @if($search)
                    <button wire:click="$set('search', '')" class="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 font-bold p-0.5">✕</button>
                @endif
            </div>
