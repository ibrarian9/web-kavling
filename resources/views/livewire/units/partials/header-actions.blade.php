    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2 flex-wrap sm:flex-nowrap shrink-0 pt-2 lg:pt-0 border-t lg:border-t-0 border-slate-100">
            ...
            @if((auth()->user()->isFounder() || auth()->user()->isFinance()) && $unit->category !== 'infrastruktur' && $unit->status === 'tersedia')
                <x-button variant="emerald" size="sm" wire:click="openDirectSppModal" title="Terbitkan Surat Pesanan SPP & SPJB PDF (Pembelian Cash)">
            ...
            @if(!auth()->user()->isPengawasProject() && $unit->category !== 'infrastruktur' && $unit->status === 'tersedia')
                <x-button variant="outline" size="sm" wire:click="openBookingModal" icon="check" title="Booking Unit Ini">
                    <span>Booking Unit Ini</span>
                </x-button>
            @endif

            @if(auth()->user()->isMarketing() && $unit->category !== 'infrastruktur' && $unit->status === 'tersedia')
                <x-button variant="primary" size="sm" href="{{ route('proposals.index', ['create_unit_id' => $unit->id]) }}" icon="plus">
                    <span>Ajukan Penawaran Harga &rarr;</span>
                </x-button>
            @endif

            @if(auth()->user()->isFounder() || auth()->user()->isFinance() || auth()->user()->isAdmin())
                <x-button variant="edit" size="sm" wire:click="openEditUnitModal" title="Edit Spesifikasi Unit">
                    <span>Edit Spesifikasi</span>
                </x-button>
            @endif

            @if(auth()->user()->isFounder())
                <x-button variant="delete" size="sm" @click="confirmModalAction({
