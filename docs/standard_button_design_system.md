...
| :--- | :--- | :--- | :--- | :--- |
| **Primary Action** | Emerald Solid (`#059669`) | `hover:bg-emerald-700` | Tambah Data, Simpan Form, Proyek Baru, Setup Skema Cicilan Baru | `btn-primary` / `btn-header-setup` atau `bg-emerald-600 hover:bg-emerald-700 text-white` |
...
| **Edit / Update** | Amber Soft (`#fffbeb`) | `bg-amber-50 hover:bg-amber-100 text-amber-800 border-amber-200` | Edit Unit, Ubah Material, Edit Gaji | `btn-action-edit` atau `bg-amber-50 text-amber-800 border border-amber-200 hover:bg-amber-100` |
| **Destructive / Hapus** | Rose Soft (`#fff1f2`) | `bg-rose-50 hover:bg-rose-100 text-rose-700 border-rose-200` | Hapus Unit, Hapus Transaksi, Batal DP | `btn-action-delete` atau `bg-rose-50 text-rose-700 border border-rose-200 hover:bg-rose-100` |
| **Lihat PDF / Document** | Sky Soft / Rose Light | `bg-sky-50 hover:bg-sky-100 text-sky-800 border-sky-200` | Lihat PDF Rekap, Unduh Resi, PDF Kuitansi | `btn-action-pdf` / `btn-header-pdf` atau `bg-sky-50 text-sky-800 border border-sky-200 hover:bg-sky-100` |
| **Detail / Audit Trail** | Teal Soft (`#f0fdf4`) | `bg-teal-50 hover:bg-teal-100 text-teal-800 border-teal-200` | Detail Alur Keuangan, Detail Skema Cicilan | `btn-action-detail` atau `bg-teal-50 text-teal-800 border border-teal-200 hover:bg-teal-100` |
| **Halaman Unit / Objek** | Slate Soft (`#f8fafc`) | `bg-slate-100 hover:bg-slate-200 text-slate-800 border-slate-200` | Link ke Halaman Unit Kavling | `btn-action-unit` atau `bg-slate-100 text-slate-800 border border-slate-200 hover:bg-slate-200` |
...
Digunakan pada kolom **Aksi**, **Resi**, atau **Audit** di tabel data (tabel mutasi kas, tabel cicilan pembeli, tabel unit, tabel belanja, dsb).
...
<!-- Tombol Setoran -->
<button class="btn-action-payment">
...
<!-- Tombol Batalkan & Ganti Cash -->
<button class="btn-action-convert">
...
Digunakan pada **Tombol Submit Form Modal** dan **Tombol Tambah Utama di Halaman Header**.
- **Height / Padding:** `px-4 py-2.5`
- **Ukuran Teks:** `text-xs sm:text-sm font-bold`
- **Border Radius:** `rounded-xl` (12px)
- **Ukuran Icon:** `w-4 h-4` atau `w-4.5 h-4.5`

#### Contoh Blade View:
```html
<!-- Tombol Setup Skema Cicilan Baru (Header Action) -->
...
.btn-action-detail {
  @apply inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-teal-50 text-teal-800 hover:bg-teal-100 border border-teal-200 text-[11px] font-bold transition shadow-2xs;
}

.btn-action-unit {
  @apply inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 text-slate-800 hover:bg-slate-200 border border-slate-200 text-[11px] font-bold transition shadow-2xs;
}

.btn-action-payment {
  @apply inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-800 hover:bg-emerald-100 border border-emerald-200 text-[11px] font-bold transition shadow-2xs;
}

.btn-action-convert {
  @apply inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-purple-50 text-purple-800 hover:bg-purple-100 border border-purple-200 text-[11px] font-bold transition shadow-2xs;
}

.btn-action-edit {
  @apply inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-amber-50 text-amber-800 hover:bg-amber-100 border border-amber-200 text-[11px] font-bold transition shadow-2xs;
}

.btn-action-delete {
  @apply inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-rose-50 text-rose-700 hover:bg-rose-100 border border-rose-200 text-[11px] font-bold transition shadow-2xs;
}

.btn-action-pdf {
  @apply inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-sky-50 text-sky-800 hover:bg-sky-100 border border-sky-200 text-[11px] font-bold transition shadow-2xs;
}
